...
async def handle_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        extract_and_save_text(pdf_path, text_path)
        ...
        if result:
            ...
            c.execute("INSERT OR REPLACE INTO user_books (user_id, book_name, last_sentence, pdf_path, text_path) VALUES (?, ?, ?, ?, ?)",
                      (user_id, file.file_name, 0, pdf_path, text_path))
            await update.message.reply_text("پی ڈی ایف موصول ہو گئی اور متن نکال لیا گیا۔ براہ کرم شروع کرنے کے لیے ایک جملہ بھیجیں۔")
        conn.commit()
        conn.close()
...
# Handle starting sentence with fuzzy matching from text file
async def handle_sentence(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        user_id = context.user_data['user_id']
        user_sentence = normalize_text(update.message.text)
        text_path = context.user_data['text_path']
        ...
        try:
            start_index = normalized_sentences.index(user_sentence)
        except ValueError:
            # If exact match fails, use fuzzy matching
            best_match_index = -1
            best_match_ratio = 0.8  # Minimum similarity threshold
            for i, norm_sentence in enumerate(normalized_sentences):
                ratio = SequenceMatcher(None, user_sentence, norm_sentence).ratio()
                if ratio > best_match_ratio:
                    best_match_ratio = ratio
                    best_match_index = i
            
            if best_match_index == -1:
                await update.message.reply_text("یہ جملہ متن میں نہیں ملا۔ براہ کرم درست جملہ بھیجیں یا دوسرا جملہ آزمائیں۔")
                return ASK_SENTENCE
            else:
                start_index = best_match_index
                await update.message.reply_text(f"دقیق میچ نہیں ملا، لیکن قریب ترین جملہ استعمال کیا جا رہا ہے: '{sentences[start_index]}'")
